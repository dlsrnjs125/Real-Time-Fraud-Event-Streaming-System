...
PaySim native type actual replay는 V2 Phase 8에서 production enum 확장 대신 mapping contract로 분리했습니다. `CASH_OUT`과 `CASH_IN`은 replay-supported normalized type으로 보내고, `DEBIT`는 unsupported로 제외합니다.
