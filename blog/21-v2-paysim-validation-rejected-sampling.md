...
Manifest를 허용하려다 `data/samples/*.json` 전체를 열면 full summary나 임의 JSON이 들어올 수 있습니다. 임의 JSONL도 수동 leakage를 만들 수 있으므로 현재 생성되는 sample 3개 파일만 제한적으로 허용합니다. Data policy check는 staged sample content도 scan해서 event label leakage, raw identifier, manifest salt value를 막습니다. 이 grep 기반 scan은 빠른 commit guardrail이고, JSON 구조 검증은 validator와 sample generator가 담당합니다.
...
- hidden/bidirectional Unicode control character scan: no matches
