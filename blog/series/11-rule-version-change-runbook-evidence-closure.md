# ruleVersion 변경 runbook과 evidence closure
...
다시 구현한다면 유지할 기준은 세 가지다. 첫째, API latency와 detection latency를 끝까지 분리한다. 둘째, Redis나 Kafka 같은 외부 시스템 장애를 “없었던 일”처럼 숨기지 않는다. 셋째, 자동화하지 않은 것은 자동화했다고 쓰지 않는다.
