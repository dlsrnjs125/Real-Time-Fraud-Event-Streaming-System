...
거래 이벤트 접수와 이상거래 탐지를 하나의 동기 요청 안에 묶으면 설명하기 어려운 지점이 생긴다. API가 느려진 원인이 요청 검증인지, Kafka 발행인지, Redis 조회인지, Rule Engine 실행인지 분리해서 볼 수 없다. 반대로 API가 빠르게 응답해도 Consumer가 밀리면 실제 탐지는 늦어진다.
...
또 하나의 결정은 Kafka partition key였다. 이상거래 탐지는 사용자별 최근 거래 순서가 중요하므로 기본 key를 `userId`로 두었다. 이 선택은 같은 사용자의 이벤트 순서를 유지하는 데 유리하지만, 특정 사용자나 테스트 데이터가 몰리면 hot partition이 생길 수 있다.
...
대표 검증은 `make final-check`로 묶었다. 이 명령은 Gradle build/test, Docker Compose config, shell script syntax, V2 data/evaluation guardrail을 확인한다. 다만 production 수준의 탐지 성능이나 latency를 보장하는 명령은 아니다.
