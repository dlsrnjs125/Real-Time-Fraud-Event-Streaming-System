...
DLT는 민감정보가 쌓이기 쉬운 저장소이기도 합니다. Phase 9에서는 payload 저장 경로를 별도 sanitizer 메서드로 분리했고, `errorMessage`는 500자로 제한하며 null/blank 메시지는 예외 class 이름으로 대체합니다. 현재 프로젝트는 synthetic identifier만 사용하므로 필드 마스킹은 적용하지 않았지만, 운영 확장 시 직접 식별자는 DLT 저장 전에 제거해야 합니다.
...
- 재처리 횟수는 기록하지만 max attempts, cooldown, 관리자 승인 정책은 구현하지 않았습니다.
- 관리자 인증/인가와 audit log는 구현하지 않았습니다.
- Kafka publish와 DB update의 atomic transaction은 구현하지 않았습니다.
- DLT payload sanitizer 경로는 분리했지만 필드별 masking rule은 운영 확장 시 보강이 필요합니다.
...
- batch reprocess, rate limit, max attempts, audit log 추가
