...
- 재처리 요청은 status를 `REPROCESSING`으로 바꾸고 `reprocess_attempts`를 증가시킨 뒤 `transaction-events`로 원본 payload를 재발행합니다.
- Kafka publish 성공 시 `REPROCESSED`, 실패 시 `REPROCESS_FAILED`로 남깁니다.
...
- reprocess status가 `REPROCESSING`으로 변경된 뒤 Kafka publish 실패: `REPROCESS_FAILED`로 상태를 남기고 운영자가 다시 판단합니다.
