...
- `fraud_detection_results.event_id` unique
...
- max attempts 초과는 자동 discard로 연결하지 않습니다. 운영자가 실패 원인을 확인하고 discard API로 명시적으로 폐기해야 합니다.
- Consumer는 `fraud_detection_results.event_id` unique constraint와 duplicate fast path로 중복 FraudResult 생성을 막습니다.

## 8. Phase 14 Audit Log와 상태 전이

DLT reprocess/discard 성공과 실패는 `admin_audit_logs`에 저장합니다.

- 성공 audit: 상태 변경과 audit 저장이 같은 transaction 안에서 수행됩니다.
...
Phase 14부터 admin API는 local/dev 최소 보호로 `X-Admin-Token`을 요구합니다. 운영 환경 가정에서는 JWT/OAuth2/RBAC, rate limit, IP allowlist, payload 접근 통제, audit log 조회 권한이 추가로 필요합니다.

Phase 14의 abuse prevention은 다음 수준입니다.

- admin token 필수화
- discard/reprocess request validation
- max reprocess attempts
- 단건 수동 조작 원칙

Gateway/Nginx/API Gateway rate limit, cooldown, batch reprocess 제한은 후속 운영 자동화 범위입니다.
