...
| V2 Phase 10 | Rule Engine V2 for PaySim Patterns | `BALANCE_DRAIN`, `ZERO_BALANCE_AFTER_TRANSFER`, `TRANSFER_CASHOUT_PATTERN` | Rule unit test와 score/risk mapping 통과 |
| V2 Phase 11 | PaySim Label-based Rule Evaluation | Java Rule Engine 기반 offline evaluation, confusion matrix, precision/recall/f1 coverage report | offline/online이 같은 rule version을 사용하고, missed/false positive 예시 문서화 |
| V2 Phase 12 | Fraud Action Decision Engine | `fraud_action_decisions`, action policy, admin query API | `unique(event_id, action_type)` 기준 action decision 생성 |
| V2 Phase 13 | Fraud Case Management | `fraud_cases`, list/detail/resolve API, audit action 확장 | HIGH/CRITICAL case 생성, resolved 상태 충돌 방어, audit log 저장 |
| V2 Phase 14 | Evidence and Visualization | result evidence doc, charts/tables, README/blog 정리 | risk/rule/action/case/confusion matrix evidence 작성 |
...
- review/block candidate count와 rate, `actionDecisionDistribution`, `operatorWorkloadSummary` 추가
- `scripts/data/verify_paysim_rule_threshold_regression.py` 추가
- `make verify-paysim-rule-threshold-regression`, `make verify-v2-phase9`, `make evaluate-paysim-threshold-regression`, `make v2-phase9-evidence` target 추가
...
- `PYTHONPYCACHEPREFIX=/private/tmp/paysim-pycache .venv-data/bin/python -m py_compile scripts/data/*.py`: PASS
- `make test-data-scripts`: PASS, 101 tests
- `make data-policy-check`: PASS
- `make verify-paysim-evaluation-report-contract`: PASS
- `make verify-paysim-native-replay-contract`: PASS
...
- `make validate-paysim`: FAIL in current local workspace because ignored `data/processed/paysim-validation-report.json` was generated by an older contract and is missing `hashAlgorithm`, `hashIdPrefixLength`. Regenerate ignored processed outputs before using this local command.
- `make evaluate-paysim-threshold-regression`: FAIL in current local workspace because ignored `data/processed/paysim-detection-results.jsonl` is not present. Export or generate local detection results before using this alias.
...
| Step 11 | V2 Phase 10 | Rule Engine V2 초기 rule 구현 |
| Step 12 | V2 Phase 11 | Rule evaluation confusion matrix |
| Step 13 | V2 Phase 12 | Action Decision Engine |
| Step 14 | V2 Phase 13 | Fraud Case Management |
| Step 15 | V2 Phase 14 | visualization/evidence, README, docs, blog 정리 |
