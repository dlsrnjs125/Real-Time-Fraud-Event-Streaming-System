...
| V2 Phase 8 | Done | PaySim native type replay contract 구현 완료 | native type mapping policy, replay/evaluation report mapping fields, CI-safe contract verifier, docs/blog | Rule Engine V2 implementation |
...
| V2 Phase 9 | Rule Engine V2 for PaySim Patterns | `BALANCE_DRAIN`, `ZERO_BALANCE_AFTER_TRANSFER`, `TRANSFER_CASHOUT_PATTERN` | Rule unit test와 score/risk mapping 통과 |
| V2 Phase 10 | PaySim Label-based Rule Evaluation | Java Rule Engine 기반 offline evaluation, confusion matrix, precision/recall/f1 coverage report | offline/online이 같은 rule version을 사용하고, missed/false positive 예시 문서화 |
| V2 Phase 11 | Fraud Action Decision Engine | `fraud_action_decisions`, action policy, admin query API | `unique(event_id, action_type)` 기준 action decision 생성 |
| V2 Phase 12 | Fraud Case Management | `fraud_cases`, list/detail/resolve API, audit action 확장 | HIGH/CRITICAL case 생성, resolved 상태 충돌 방어, audit log 저장 |
| V2 Phase 13 | Evidence and Visualization | result evidence doc, charts/tables, README/blog 정리 | risk/rule/action/case/confusion matrix evidence 작성 |
...
| Step 10 | V2 Phase 9 | Rule Engine V2 초기 rule 구현 |
| Step 11 | V2 Phase 10 | Rule evaluation confusion matrix |
| Step 12 | V2 Phase 11 | Action Decision Engine |
| Step 13 | V2 Phase 12 | Fraud Case Management |
| Step 14 | V2 Phase 13 | visualization/evidence, README, docs, blog 정리 |
