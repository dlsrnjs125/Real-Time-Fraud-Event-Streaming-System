...
| V2 Phase 11 | Rule Engine V2 for PaySim Patterns | `BALANCE_DRAIN`, `ZERO_BALANCE_AFTER_TRANSFER`, `TRANSFER_CASHOUT_PATTERN` | Rule unit test와 score/risk mapping 통과 |
| V2 Phase 12 | PaySim Label-based Rule Evaluation | Java Rule Engine 기반 offline evaluation, confusion matrix, precision/recall/f1 coverage report | offline/online이 같은 rule version을 사용하고, missed/false positive 예시 문서화 |
| V2 Phase 13 | Fraud Action Decision Engine | `fraud_action_decisions`, action policy, admin query API | `unique(event_id, action_type)` 기준 action decision 생성 |
| V2 Phase 14 | Fraud Case Management | `fraud_cases`, list/detail/resolve API, audit action 확장 | HIGH/CRITICAL case 생성, resolved 상태 충돌 방어, audit log 저장 |
| V2 Phase 15 | Evidence and Visualization | result evidence doc, charts/tables, README/blog 정리 | risk/rule/action/case/confusion matrix evidence 작성 |
...
- blog index 갱신
- V2 다음 단계에서 Rule Engine V2 구현을 Phase 11로 이동
...
| Step 12 | V2 Phase 11 | Rule Engine V2 초기 rule 구현 |
| Step 13 | V2 Phase 12 | Rule evaluation confusion matrix |
| Step 14 | V2 Phase 13 | Action Decision Engine |
| Step 15 | V2 Phase 14 | Fraud Case Management |
| Step 16 | V2 Phase 15 | visualization/evidence, README, docs, blog 정리 |
