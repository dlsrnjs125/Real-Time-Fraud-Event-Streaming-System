...
| V2 Phase 12 | Rule Engine V2 for PaySim Patterns | `BALANCE_DRAIN`, `ZERO_BALANCE_AFTER_TRANSFER`, `TRANSFER_CASHOUT_PATTERN` | Rule unit test와 score/risk mapping 통과 |
| V2 Phase 13 | PaySim Label-based Rule Evaluation | Java Rule Engine 기반 offline evaluation, confusion matrix, precision/recall/f1 coverage report | offline/online이 같은 rule version을 사용하고, missed/false positive 예시 문서화 |
| V2 Phase 14 | Fraud Action Decision Engine | `fraud_action_decisions`, action policy, admin query API | `unique(event_id, action_type)` 기준 action decision 생성 |
| V2 Phase 15 | Fraud Case Management | `fraud_cases`, list/detail/resolve API, audit action 확장 | HIGH/CRITICAL case 생성, resolved 상태 충돌 방어, audit log 저장 |
| V2 Phase 16 | Evidence and Visualization | result evidence doc, charts/tables, README/blog 정리 | risk/rule/action/case/confusion matrix evidence 작성 |
...
- `blog/28-v2-final-readiness-and-readme-slimdown.md` 추가
- blog index 갱신
...
- evaluation report에 `ruleVersionDistribution` 추가
...
- fixture report에서 per-result `ruleVersion` coverage와 distribution을 검증
...
- `FraudResult`에 per-result `ruleVersion`을 저장하지 않았습니다.
...
| Step 13 | V2 Phase 12 | Rule Engine V2 초기 rule 구현 |
| Step 14 | V2 Phase 13 | Rule evaluation confusion matrix |
| Step 15 | V2 Phase 14 | Action Decision Engine |
| Step 16 | V2 Phase 15 | Fraud Case Management |
| Step 17 | V2 Phase 16 | visualization/evidence, README, docs, blog 정리 |
