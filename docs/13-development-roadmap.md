...
| V2 Phase 6 | Done | PaySim replay result evaluation baseline 구현 완료 | evaluation script, detection result export contract, confusion matrix/metrics report, fixture unittest, docs/blog | V2 Phase 7 Rule Engine V2 implementation |
...
| V2 Phase 7 | Rule Engine V2 for PaySim Patterns | `BALANCE_DRAIN`, `ZERO_BALANCE_AFTER_TRANSFER`, `TRANSFER_CASHOUT_PATTERN` | Rule unit test와 score/risk mapping 통과 |
| V2 Phase 8 | PaySim Label-based Rule Evaluation | Java Rule Engine 기반 offline evaluation, confusion matrix, precision/recall/f1 coverage report | offline/online이 같은 rule version을 사용하고, missed/false positive 예시 문서화 |
| V2 Phase 9 | Fraud Action Decision Engine | `fraud_action_decisions`, action policy, admin query API | `unique(event_id, action_type)` 기준 action decision 생성 |
| V2 Phase 10 | Fraud Case Management | `fraud_cases`, list/detail/resolve API, audit action 확장 | HIGH/CRITICAL case 생성, resolved 상태 충돌 방어, audit log 저장 |
| V2 Phase 11 | Evidence and Visualization | result evidence doc, charts/tables, README/blog 정리 | risk/rule/action/case/confusion matrix evidence 작성 |
...
- Java Rule Engine V2는 구현하지 않았습니다.
- PaySim native `CASH_OUT`, `CASH_IN`, `DEBIT` actual replay는 Phase 7 이후 API DTO 확장 또는 Rule V2 contract와 함께 처리합니다.
...
- `bash -n scripts/data/*.sh`: PASS
- `make data-env`: PASS
- `python3 -m unittest discover -s scripts/data -p 'test_*.py'`: PASS, 82 tests
- `make test-data-scripts`: PASS, 82 tests
- `make data-policy-check`: PASS
- `make ci-check`: PASS
- `make replay-paysim-sample-dry-run`: PASS, read=100 accepted=76 rejected=24
...
| Step 8 | V2 Phase 7 | Rule Engine V2 초기 rule 구현 |
| Step 9 | V2 Phase 8 | Rule evaluation confusion matrix |
| Step 10 | V2 Phase 9 | Action Decision Engine |
| Step 11 | V2 Phase 10 | Fraud Case Management |
| Step 12 | V2 Phase 11 | visualization/evidence, README, docs, blog 정리 |
