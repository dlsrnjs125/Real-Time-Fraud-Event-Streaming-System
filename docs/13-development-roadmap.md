...
- evaluation report에 `reportSchemaVersion`, `f1Score`, `totalEvents`, `fraudLabeledEvents`, `detectedFraudEvents`, `missedFraudEvents`, `falsePositiveEvents`, `truePositiveEvents`, `trueNegativeEvents`, `misclassifiedEvents`, `unmatchedResultEvents`, `evaluationExcludedRecords` 추가
- `failedRecords`, `invalidRecords`는 탐지 오분류가 아니라 pipeline/schema failure 의미로 분리
