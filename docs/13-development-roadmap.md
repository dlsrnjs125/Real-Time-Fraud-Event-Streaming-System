...
| V2 Phase 7 | Done | PaySim replay evaluation evidence 정리 완료 | evidence doc, troubleshooting/index update, Makefile alias, evaluator metric aliases, blog draft | Rule Engine V2 implementation |
...
| V2 Phase 8 | Rule Engine V2 for PaySim Patterns | `BALANCE_DRAIN`, `ZERO_BALANCE_AFTER_TRANSFER`, `TRANSFER_CASHOUT_PATTERN` | Rule unit test와 score/risk mapping 통과 |
| V2 Phase 9 | PaySim Label-based Rule Evaluation | Java Rule Engine 기반 offline evaluation, confusion matrix, precision/recall/f1 coverage report | offline/online이 같은 rule version을 사용하고, missed/false positive 예시 문서화 |
| V2 Phase 10 | Fraud Action Decision Engine | `fraud_action_decisions`, action policy, admin query API | `unique(event_id, action_type)` 기준 action decision 생성 |
| V2 Phase 11 | Fraud Case Management | `fraud_cases`, list/detail/resolve API, audit action 확장 | HIGH/CRITICAL case 생성, resolved 상태 충돌 방어, audit log 저장 |
| V2 Phase 12 | Evidence and Visualization | result evidence doc, charts/tables, README/blog 정리 | risk/rule/action/case/confusion matrix evidence 작성 |
...
- Java Rule Engine V2는 구현하지 않았습니다.
- PaySim native `CASH_OUT`, `CASH_IN`, `DEBIT` actual replay는 V2 Phase 8 이후 API DTO 확장 또는 Rule V2 contract와 함께 처리합니다.
- `--event-type-policy preserve`는 native type을 사전 rejected로 집계하지 않으며, current API가 거부하면 HTTP 4xx/client error로 기록합니다.
...
- `blog/25-v2-paysim-replay-evaluation-evidence.md` 추가
- `make evaluate-paysim-replay`, `make verify-v2-phase7`, `make v2-phase7-evidence` target 추가
- evaluation report에 `f1Score`, `totalEvents`, `fraudLabeledEvents`, `detectedFraudEvents`, `missedFraudEvents`, `falsePositiveEvents`, `truePositiveEvents`, `trueNegativeEvents`, `failedRecords`, `invalidRecords` 추가
...
- CI-safe verification command가 full PaySim raw data나 local DB export 없이 실행 가능
...
- `make test-data-scripts`: PASS, 87 tests
- `make data-policy-check`: PASS
- `make verify-v2-phase7`: PASS, 87 data script tests + data policy check
- `./gradlew test`: PASS
- `make final-check`: PASS after allowing Gradle/Docker access outside the restricted sandbox
- `make validate-paysim`: FAIL in current local workspace because ignored `data/processed/paysim-validation-report.json` was generated by an older contract and is missing `hashAlgorithm`, `hashIdPrefixLength`
...
| Step 9 | V2 Phase 8 | Rule Engine V2 초기 rule 구현 |
| Step 10 | V2 Phase 9 | Rule evaluation confusion matrix |
| Step 11 | V2 Phase 10 | Action Decision Engine |
| Step 12 | V2 Phase 11 | Fraud Case Management |
| Step 13 | V2 Phase 12 | visualization/evidence, README, docs, blog 정리 |
...
V2 Phase 1~3과 Data Toolchain까지 구현했습니다. 현재 구현된 V2 범위는 data guardrail, KaggleHub raw acquisition helper, PaySim preprocessing normalization, processed output validation, safe sample generation입니다. Java Rule Engine V2, replay pipeline, DB migration, API 추가는 아직 구현하지 않았습니다.
