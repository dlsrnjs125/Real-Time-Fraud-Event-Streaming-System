...
## V2 Phase 7. Replay evaluation 결과를 탐지 성능으로 과대 해석할 위험

### 문제

PaySim replay evaluation report의 precision/recall 숫자를 실제 운영 fraud detection 성능처럼 설명할 수 있습니다.

### 판단

V2 Phase 7은 성능 자랑이 아니라 baseline과 재현성 확보 단계입니다. PaySim은 synthetic dataset이고, 현재 결과는 local detection result export와 label sidecar join으로 만든 rule baseline evidence입니다.

### 대응

`docs/31-v2-replay-evaluation-evidence.md`에서 precision/recall 해석 범위, synthetic dataset 한계, rule-based detection 한계를 분리했습니다.

README에는 다음 취지를 짧게만 남깁니다.

```text
V2 replay evaluation is baseline evidence for rule/report verification, not a production fraud performance guarantee.
```

### 남은 한계

실제 운영 성능을 말하려면 운영 데이터 검증, drift monitoring, rule/model validation, human review 결과가 필요합니다.

## V2 Phase 7. 평가 지표와 운영 지표의 혼동

### 문제

precision/recall과 p95 latency, Consumer Lag, DLQ count가 같은 성격의 지표처럼 섞일 수 있습니다.

### 판단

Detection quality metric과 streaming operation metric은 질문이 다릅니다. Precision/recall은 PaySim label과 rule output의 agreement를 설명하고, Consumer Lag과 detection latency는 비동기 처리 상태를 설명합니다.

### 대응

Phase 7 evidence 문서에서 평가 지표와 운영 지표를 분리했습니다. Gate 가능 기준도 script/report contract와 운영 성능 보류 기준으로 나눴습니다.

README에는 다음 취지를 짧게만 남깁니다.

```text
Detection quality and streaming operation metrics are interpreted separately.
```

### 남은 한계

Grafana dashboard와 replay evaluation report를 하나의 evidence bundle로 묶는 작업은 후속 Phase에서 다룹니다.

## V2 Phase 7. 대용량 PaySim 결과 커밋 위험

### 문제

Raw CSV, full processed JSONL, local evaluation report가 repository에 들어가면 파일 크기와 데이터 관리 기준을 동시에 깨뜨릴 수 있습니다.

### 판단

Repository에는 작은 smoke/sample data만 허용합니다. Raw/full processed PaySim data와 local report는 재현 가능한 command와 manifest로 설명하고 Git에는 남기지 않습니다.

### 대응

`.gitignore`, `make data-policy-check`, `docs/24-kaggle-paysim-data-provenance.md`, `docs/31-v2-replay-evaluation-evidence.md` 기준을 연결했습니다.
...
Data policy check는 Git guardrail입니다. 로컬 디렉터리에 이미 존재하는 대용량 파일 자체를 삭제하지는 않습니다.

## V2 Phase 7. Threshold 조정과 false positive 증가

### 문제

Rule threshold를 낮추면 recall이 좋아진 것처럼 보일 수 있지만, false positive가 늘어 운영자 workload가 커질 수 있습니다.

### 판단

Fraud detection에서는 false negative가 위험하지만, false positive도 운영 비용과 사용자 영향으로 이어집니다. Threshold 변경은 precision, recall, false positive count, action decision distribution을 함께 봐야 합니다.

### 대응

Phase 7 evidence 문서에 precision/recall 해석과 operator workload trade-off를 기록했습니다.

README에는 다음 취지를 짧게만 남깁니다.

```text
Rule threshold changes are reviewed with precision, recall, and operator workload together.
```
...
## V2 Phase 11 evaluation report ruleVersion이 실제 consumer rule과 연결되지 않는 문제

문제:
Evaluation report의 `ruleVersion`이 Python evaluator 안에서만 관리되면 Java app-consumer Rule Engine baseline과 다르게 바뀌어도 report만 보면 알기 어렵습니다.

판단:
Metric 비교에는 rule logic source와 evaluation policy가 같은 baseline을 가리킨다는 증거가 필요합니다. Phase 11에서는 새 rule을 추가하지 않고 version drift를 먼저 막습니다.

해결:
`FraudRuleVersions.java`에 app-consumer baseline `rule-v2-baseline-v1`을 두고, `verify_paysim_rule_version_contract.py`가 Python `RULE_VERSION`과 직접 비교합니다.

## V2 Phase 11 ruleVersion과 thresholdVersion을 섞는 문제

문제:
`ruleVersion`과 `thresholdVersion`을 하나의 버전처럼 다루면 metric 변화 원인을 분리하기 어렵습니다. Rule logic이 바뀐 것인지, HIGH/MEDIUM threshold 경계가 바뀐 것인지 알 수 없습니다.

판단:
Rule logic version과 threshold boundary version은 서로 다른 계약입니다. 둘 다 precision/recall/F1에 영향을 줄 수 있지만 review 기준은 달라야 합니다.

해결:
Phase 11 verifier는 `ruleVersion`만 Java baseline과 비교하고, threshold fallback과 workload 검증은 Phase 9 `verify-paysim-rule-threshold-regression`에 남깁니다.

## V2 Phase 11 Java/Python version drift 문제

문제:
Java에서 rule baseline을 변경하고 Python evaluator policy를 갱신하지 않거나, 반대로 Python만 바꾸면 CI fixture report가 실제 consumer rule 의미와 어긋날 수 있습니다.

판단:
Drift는 full PaySim replay를 기다리지 않고 작은 fixture verifier에서 fail-fast 되는 편이 안전합니다.

해결:
`make verify-paysim-rule-version-contract`가 Java source constant, Python allowlist, evaluator report output, mismatched per-result failure를 함께 확인합니다. `make final-check`는 Phase 11 aggregate check를 통해 이 verifier를 포함합니다.

## V2 Phase 11 per-result ruleVersion 없이 이벤트별 rule consistency를 보장하는 것처럼 말하는 문제

문제:
Detection result export row에 `ruleVersion`이 없는데도 모든 event가 특정 Java rule version으로 처리됐다고 말하면 과장입니다.

판단:
Phase 11은 contract-level version alignment를 검증합니다. Event별 처리 결과가 어떤 ruleVersion에서 나왔는지는 per-result persistence/export가 있어야 더 강하게 말할 수 있습니다.

해결:
Evaluator는 per-result `ruleVersion`이 있으면 반드시 expected version과 일치해야 하고, 없으면 `ruleVersionCoverage`와 warning을 남깁니다. Per-result export는 후속 hardening으로 분리합니다.

## V2 Phase 11 rule version integration을 fraud 성능 개선으로 오해하는 문제

문제:
Rule version integration이 추가되면 fraud detection quality가 좋아진 것처럼 읽힐 수 있습니다.

판단:
이번 작업은 evidence reproducibility와 drift detection입니다. 새로운 fraud rule, threshold tuning, model baseline, production performance 검증은 포함하지 않습니다.

해결:
`docs/35-v2-rule-version-integration-evidence.md`와 review 기록에 "version consistency check, not production fraud model performance"를 명시합니다.

## V2 Phase 12 report-level ruleVersion만 보고 row-level consistency를 과장하는 문제

문제:
Report-level `ruleVersion`은 evaluation contract가 어떤 baseline을 사용했는지 설명하지만, 각 detection result row가 실제로 같은 ruleVersion으로 생성됐는지는 약하게만 설명합니다.

판단:
Row-level consistency를 말하려면 per-result `ruleVersion` propagation, coverage, distribution, strict mode가 필요합니다.

해결:
신규 `FraudRuleEngineResult`와 `fraud_detection_results` 저장 경로에 active `ruleVersion`을 전파하고, evaluator는 `ruleVersionCoverage`, `ruleVersionReadiness`, `ruleVersionDistribution`을 기록합니다.

## V2 Phase 12 legacy export에 ruleVersion이 없어 strict mode가 실패하는 문제

문제:
Phase 11 이전 export나 fixture에는 per-result `ruleVersion`이 없을 수 있습니다. 이 상태에서 strict mode를 기본값으로 두면 legacy evidence가 모두 실패합니다.

판단:
Default compatibility mode와 strict evidence mode를 분리해야 합니다.

해결:
기본 evaluator mode는 missing `ruleVersion`을 warning과 coverage로 허용합니다. `--require-per-result-rule-version`을 명시한 경우에만 missing evaluated row를 fail-fast 처리합니다.

## V2 Phase 12 mixed present/missing ruleVersion에서 distribution이 오염되는 문제

문제:
일부 row에만 `ruleVersion`이 있을 때 missing 값을 `ruleVersionDistribution`에 넣으면 `null` 또는 `None` key가 생겨 report consumer가 혼동할 수 있습니다.

판단:
Distribution은 실제 present version의 분포만 표현해야 합니다. Missing count는 coverage field가 담당합니다.

해결:
`ruleVersionDistribution`은 present per-result version만 집계하고, missing row는 `ruleVersionCoverage.resultsWithoutRuleVersion`으로만 기록합니다. Phase 12 verifier가 mixed fixture에서 이 동작을 검증합니다.

## V2 Phase 12 ruleVersion persistence 범위를 과장하는 문제

문제:
Rule Engine result에만 `ruleVersion`이 있거나 DB column만 추가됐는데, export/backfill까지 완료된 것처럼 문서화하면 evidence 신뢰도가 떨어집니다.

판단:
In-memory result, persistence, API response, export automation, historical backfill은 각각 다른 완료 범위입니다.

해결:
Phase 12는 신규 detection result 저장과 admin query response까지 구현했습니다. Historical backfill과 automated DB detection result export는 future work로 분리했습니다.

## V2 Phase 12 reportSchemaVersion을 올리지 않고 새 필드를 추가하는 문제

문제:
`requirePerResultRuleVersion`이나 `ruleVersionReadiness` 같은 report field가 추가됐는데 schema version이 그대로면 downstream verifier가 오래된 contract로 새 report를 해석할 수 있습니다.

판단:
Report field가 contract 의미를 바꾸면 `reportSchemaVersion`을 올리고 fixture verifiers를 함께 갱신해야 합니다.
...
## V2 Phase 15 최종 요약이 README를 다시 비대하게 만드는 문제

문제:
최종 evidence 요약을 README에 넣으면 README가 다시 phase history, command matrix, troubleshooting 요약으로 길어질 수 있습니다.
...
문제:
검증 명령이 많아지면 `make final-check`가 실제 운영 성능이나 탐지 품질을 보장한다고 오해할 수 있습니다.

판단:
`make final-check`는 readiness guardrail입니다.

해결:
Final summary의 Verification Map에서 `make final-check`가 검증하는 것과 검증하지 않는 것을 분리합니다.

## V2 Phase 15 implemented와 future work가 섞이는 문제

문제:
최종 요약에서 dashboard, automatic rollback, historical backfill 같은 future work가 완료된 것처럼 보이면 신뢰도가 떨어집니다.

판단:
Implemented, local/manual, future work를 분리해야 합니다.

해결:
Final summary에 세 섹션을 따로 두고, 완료되지 않은 작업은 future work로만 기록합니다.

## V2 Phase 15 ruleVersion traceability를 fraud detection quality로 과장하는 문제

문제:
`ruleVersion`이 잘 추적된다고 탐지 정확도나 false positive rate가 개선된 것은 아닙니다.

판단:
Traceability evidence와 model quality evidence를 분리해야 합니다.

해결:
Anti-overclaim guardrails에 ruleVersion traceability와 detection accuracy가 다르다고 명시합니다.

## V2 Phase 15 Core Phase 14와 V2 Phase 15 numbering이 혼동되는 문제

문제:
Root README에는 core streaming pipeline Phase 14가 있고, 이번 작업은 V2 Phase 15입니다. 두 phase 체계를 섞으면 범위가 혼동됩니다.

판단:
V2 문서에서는 항상 V2 prefix를 유지합니다.
