...
특정 거래 이벤트의 탐지 결과를 조회합니다. fraud result 조회 API는 운영자용 admin API로 정의했습니다. 현재 Phase에서는 인증/인가 구현보다 처리 결과 저장과 조회 계약 검증에 집중했으며, 실제 운영 확장 시 ADMIN 권한 기반 접근 제어와 감사 로그를 추가합니다.
...
`matched_rules`와 `skipped_rules`는 DB에 text로 저장하고 API response에서 배열로 변환합니다. Phase 6부터 Redis 장애 시 `degraded=true`와 Redis 의존 skipped rule을 조회할 수 있습니다. raw Kafka payload는 응답하지 않습니다.
...
| `ruleCode` | `RAPID_TRANSACTION_COUNT` | matched 또는 skipped rule code |
| `page` | `0` | page index |
| `size` | `20` | page size |
  ...
  "items": [
    {
      ...
      "riskLevel": "HIGH",
      "riskScore": 75,
      "matchedRuleCodes": ["AMOUNT_THRESHOLD", "RAPID_TRANSACTION_COUNT"],
      "skippedRuleCodes": [],
      "degraded": false,
...
```json
{
  ...
  "ruleResults": [
    ...
    {
      "ruleCode": "RAPID_TRANSACTION_COUNT",
      "matched": true,
      "skipped": false,
      "score": 30,
...
```json
{
  "items": [
    ...
    },
    {
      ...
      "enabled": true,
      "score": 30,
      "condition": "5 transactions within 5 minutes",
      "requiresRedis": true
    }
  ]
}
...
Phase 14부터 `/api/v1/admin/**` API에는 local/dev 최소 보호로 `X-Admin-Token` header가 필요합니다.

- Header: `X-Admin-Token: <token>`
- 설정: `security.admin.token` 또는 `ADMIN_API_TOKEN`
- token 없음/오류: `401 UNAUTHORIZED_ADMIN_API`
- 일반 transaction event ingest API는 token 없이 동작합니다.
...
운영 환경 노출 전에는 JWT/OAuth2, ADMIN role, RBAC, audit log 조회 권한, rate limit, payload 접근 제어가 필요합니다.

- Admin API는 여전히 local/development security model입니다.
- DLQ payload와 민감 식별자 원문 조회는 기본 API에서 제공하지 않습니다.

### Phase 14 DLT 운영 API 변경
...
- 기본값: `3`
- `PENDING`, `REPROCESS_FAILED`만 재처리 가능합니다.
- `reprocess_attempts >= maxAttempts`이면 `409 MAX_REPROCESS_ATTEMPTS_EXCEEDED`로 응답하고 Kafka publish를 호출하지 않습니다.
- max attempts 초과가 자동 discard로 이어지지는 않습니다. 운영자가 별도 discard API로 명시적으로 폐기해야 합니다.

DLT discard request validation:
...
| Phase 5 | `GET /api/v1/admin/events/{eventId}/fraud-result` | Rule Engine v1 결과 조회, duplicate eventId 방어 확인 |
| Phase 6 | fraud result 상세의 `matchedRules` | Redis Sliding Window rule 결과 조회 가능 |
| Phase 6 | fraud result 상세의 `skippedRules`, `degraded` | Redis 장애 시 degraded 결과 조회 가능 |
| Phase 9 | DLQ 조회/재처리/폐기 API, processing log API | DLQ metadata 저장, 재처리 상태 전이, 중복 FraudResult 미생성 |
| Phase 10 | operational readiness summary | API/Consumer/Kafka/Redis/PostgreSQL/DLT 확인 기준 문서화 |
| Phase 11 | final readiness checklist, evidence index | Phase 1~10 evidence와 follow-up 후보 분리 |
| Phase 14 | admin API protection and audit | `X-Admin-Token`, audit log, max reprocess attempts |
| V2 Planning | PaySim runtime event and admin workflow | `TransactionBalanceFeatures`, action decision list, fraud case API |

Phase 번호는 `docs/13-development-roadmap.md`와 함께 유지합니다.

## 15. V2 PaySim API Planning

V2 PaySim 구현 전 설계 기준입니다. 아직 API DTO가 구현된 상태는 아닙니다.

### Transaction event intake extension

V2 replay payload는 runtime event만 포함합니다.

포함:

- 기존 transaction event fields
- `destinationAccountId`
- `source`
- `TransactionBalanceFeatures balanceFeatures`
...
`GET /api/v1/admin/events/{eventId}/action-decision`은 단건 action이 아니라 action list를 반환합니다. CRITICAL 이벤트는 `BLOCK_TRANSACTION_CANDIDATE`와 `ACCOUNT_RISK_FLAG`를 함께 가질 수 있기 때문입니다.

### Fraud case API

V2 fraud case API는 list/detail/resolve를 분리합니다.

- `GET /api/v1/admin/fraud-cases?status=OPEN&page=0&size=20`
