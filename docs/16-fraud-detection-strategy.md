...
Phase 5의 rule threshold는 코드 상수입니다. 운영 중 threshold 변경, rule versioning, A/B 검증은 후속 과제로 둡니다.
...
| 조건 | 최근 60초 내 거래 5건 이상 |
| 점수 | +35 |
| 필요 데이터 | 사용자별 최근 거래 이벤트 |
| Redis 필요 여부 | 필요 |
| Redis 구조 | `ZSET fraud:velocity:{userId}` |
