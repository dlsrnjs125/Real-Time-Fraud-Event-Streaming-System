...
## 21. V2 PaySim Identifier Hashing and Salt Policy

PaySim은 synthetic dataset이지만 `nameOrig`, `nameDest`는 account-like identifier입니다. V2 data workflow에서는 실제 개인정보 여부와 무관하게 replay/sample/log 경계에서 민감 식별자처럼 취급합니다.

기준:

- raw `nameOrig`, `nameDest`는 processed event, label, rejected row, validation report, sample manifest, committed sample, replay payload, log, metric tag에 남기지 않습니다.
- normalized event identifier는 HMAC-SHA256 pseudonym을 사용합니다.
- identifier format은 `userId=U-<16 lowercase hex>`, `accountId=A-<16 lowercase hex>`, `destinationAccountId=D-<16 lowercase hex>`입니다.
- salt는 secret으로 관리하고 Git, docs, logs, validation report, sample manifest에 값 자체를 남기지 않습니다.
- report와 manifest에는 `hashSaltSource`, `hashAlgorithm`, `hashIdPrefixLength`만 기록합니다.
- `default-local` salt는 local smoke/debug 용도이며 committed/shared sample에는 사용하지 않습니다.
- shared sample 재생성은 `PAYSIM_HASH_SALT` 환경 변수와 strict target을 사용합니다.
- `--hash-salt`는 shell history에 남을 수 있으므로 로컬 재현성 검증용으로만 사용하고, 공유/커밋 sample 재생성에는 환경 변수 방식을 권장합니다.

HMAC pseudonymization은 raw identifier 노출 위험을 낮추기 위한 조치이며 완전한 익명화가 아닙니다. 같은 salt와 input을 가진 사람은 같은 pseudonym을 재생성할 수 있으므로 salt 접근 권한을 별도 secret처럼 관리해야 합니다.

`--require-non-default-salt`는 `default-local` source 사용을 막는 최소 정책입니다. Salt entropy, rotation, 보관 위치, 접근 권한은 별도 운영 secret 정책으로 관리해야 합니다.
