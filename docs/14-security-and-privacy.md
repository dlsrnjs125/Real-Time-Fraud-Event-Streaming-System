...
`errorMessage`에는 stacktrace 전체를 저장하지 않습니다. DLT topic payload와 `dead_letter_events.payload_json`은 운영자 복구에 필요한 최소 범위로 제한해야 하며, 운영 환경 노출 전 관리자 인증/인가와 감사 로그를 추가해야 합니다.
