...
- API Requests by Status: `sum by (status) (increase(http_server_requests_seconds_count{job="app-api"}[$__range]))`
- API and Consumer Request Rate: `sum by (job) (rate(http_server_requests_seconds_count[1m]))`
...
- Detection Processing Latency: `max(fraud_detection_processing_latency_seconds_max)`
- DLT Operation Counters: `fraud_dlt_published_total`, `fraud_dlt_reprocess_requested_total`, `fraud_dlt_discarded_total`
- API p95 Latency: `histogram_quantile(0.95, sum by (le) (rate(http_server_requests_seconds_bucket{job="app-api"}[1m])))`
