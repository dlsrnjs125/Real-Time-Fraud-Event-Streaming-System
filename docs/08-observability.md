...
`app-api`와 `app-consumer`는 Spring Boot Actuator와 Micrometer Prometheus registry를 통해 `/actuator/prometheus` endpoint를 노출합니다. Kafka consumer group lag은 로컬 Docker Compose의 `kafka-exporter`가 Kafka broker에서 수집하고 Prometheus가 `kafka-exporter:9308`을 scrape합니다.

Prometheus는 두 애플리케이션과 Kafka exporter metric을 scrape하고, Grafana dashboard에서 API/Consumer/Kafka/Redis 상태를 확인합니다.
...
| `fraud.api.intake.service.latency` | Histogram Timer | app-api intake service method-body 처리 시간 |
| `fraud.api.intake.transaction.latency` | Histogram Timer | intake service 진입부터 Spring transaction completion callback까지의 처리 시간 |
...
| `fraud.api.receipt.status.update.latency` | Histogram Timer | publish success/failure 상태 save-call 경계 |
...
V3 Phase 1에서는 app-api intake의 known constraint도 분리합니다. `fraud.api.intake.service.latency`는 service method body 경계이고, `fraud.api.intake.transaction.latency`는 `TransactionSynchronization.afterCompletion`에서 기록하는 transaction-inclusive 경계입니다. `fraud.api.receipt.status.update.latency`는 JPA save call duration이며 실제 DB commit 또는 UPDATE round trip 전체를 의미하지 않습니다.

`fraud.kafka.publish.wait.latency`가 증가하면서 `fraud.api.intake.transaction.latency`와 Hikari pending이 함께 증가하면, Kafka ACK 대기가 DB transaction과 connection 점유를 연장하는 causal chain 후보로 분류합니다. 이 Timer들은 구조 변경 evidence를 만들기 위한 측정 경계이며, Outbox나 commit-after-publish 전환을 미리 가정하지 않습니다.
...
DLT Operation Counters는 DLT publish/reprocess/discard 이벤트가 발생한 뒤에 증가합니다. 정상 duplicate replay나 Redis down drill만 실행한 dashboard에서는 No data가 정상일 수 있습니다. DLT drill 성공 후에는 Bar gauge panel에서 Admin discard operation counter 증가를 확인합니다. `fraud.dlt.published.total`은 unique DLT backlog count가 아니라 DLT envelope publish success count입니다. DLT backlog/status count는 future work이고, DLT evidence는 별도 DLT admin audit response screenshot과 함께 해석합니다.
...
V3 Phase 7 uses existing low-cardinality stream metrics with topic/group separation and Prometheus target labels rather than adding event-level replay tags. Live and replay are distinguished by:
...
- Retry/DLT metric과 Kafka Consumer Lag dashboard는 local evidence dashboard에 연결했습니다. DLQ status/backlog gauge는 후속 Phase에서 연결합니다.
...
Kafka Consumer Lag은 local dashboard에서 `kafka_consumergroup_lag`로 확인합니다. Phase 12에서는 Kafka lag panel, 처리 결과 조회, processing log, degraded metric을 함께 사용해 비동기 처리 영향을 판단합니다.
...
Consumer Lag metric은 local `kafka-exporter`와 Grafana panel로 확인합니다. Phase 13에서는 Kafka Consumer Lag panel, processing log, fraud result 조회, Redis degraded metric을 함께 사용해 비동기 처리 영향을 보조적으로 판단합니다.
