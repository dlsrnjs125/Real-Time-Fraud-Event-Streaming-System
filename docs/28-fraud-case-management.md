...
| CRITICAL | ACCOUNT_RISK_FLAG | 별도 action decision row로 유지 |
...
FraudCase는 `eventId`를 통해 관련 ActionDecision 목록을 조회합니다. `ACCOUNT_RISK_FLAG`는 case table에 중복 저장하지 않고 action decision row로 유지합니다.
...
자동 ActionDecision 생성은 `admin_audit_logs`에 모두 기록하지 않습니다. `fraud_action_decisions` table이 system decision record 역할을 하고, admin audit log는 운영자 직접 조치에 집중합니다.
