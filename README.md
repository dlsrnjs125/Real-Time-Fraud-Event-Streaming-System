...
V3 Phase 0의 dataset profile, versioned workload manifest, stream-stage metrics, timestamp decision, Grafana dashboard, low-rate baseline evidence를 완료했습니다. V3 Phase 1에서는 sustainable throughput과 backlog recovery를 측정했고, V3 Phase 2에서는 Redis sliding-window state-size가 Redis/Consumer latency에 미치는 영향을 동일 workload 조건에서 측정했습니다. V3 Phase 3~5에서는 partition skew, redelivery/stateful idempotency, event-time lateness semantics를 검증했습니다. V3 Phase 6에서는 Organic Burst와 Catch-up Burst를 같은 300 EPS/9,000-event workload에서 비교해 pre-ingress event age와 downstream Kafka/Consumer/Redis/Sink 지연을 분리했습니다. V3 Phase 7은 historical replay가 live topic/group/Redis state를 오염시키지 않도록 replay topic, replay consumer group, Redis namespace, replay workload foundation을 구현 중입니다. 상세 구현과 측정 한계는 [V3 Phase 0 Foundation Evidence](docs/44-v3-phase0-foundation-evidence.md), [V3 Phase 1 Sustainable Throughput Evidence](docs/46-v3-phase1-sustainable-throughput-evidence.md), [V3 Phase 2 Stateful Sliding-Window Evidence](docs/48-v3-phase2-stateful-window-evidence.md), [V3 Phase 6 External Delay and Catch-up Burst Plan](docs/52-v3-phase6-external-delay-catchup-plan.md), [V3 Phase 6 Runtime Evidence](docs/evidence/v3-phase6/README.md), [V3 Phase 7 Historical Replay Isolation Plan](docs/53-v3-phase7-historical-replay-isolation-plan.md)를 기준으로 합니다.
